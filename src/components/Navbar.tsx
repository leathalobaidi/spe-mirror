...
export default function Navbar() {
  ...
  const textColor = isTransparent ? 'text-white' : 'text-spe-ink'
  const textMuted = isTransparent ? 'text-white/70' : 'text-spe-muted'
  const hoverBg = isTransparent ? 'hover:bg-white/10' : 'hover:bg-spe-paper'
  const activeLinkClass = isTransparent ? 'bg-white/15 text-white' : 'bg-spe-blue/8 text-spe-blue'
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      ...
      <nav className={`transition-all duration-500 ${navBg}`} aria-label="Main navigation">
          ...
          <div className="flex items-center justify-between h-14">
              ...
              {navItems.map(item => (
                ...
                >
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `px-3.5 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-1 ${
                        isActive ? activeLinkClass : `${textMuted} ${hoverBg}`
                      }`
                    }
                  >
                    {item.label}
                    {item.hasMega && (
                      <svg className={`w-3.5 h-3.5 transition-transform duration-300 ${activeMega === item.label ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  ...
                  {item.hasMega && megaMenus[item.label] && (
                    <div
                      className={`absolute top-full left-0 pt-2 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                        activeMega === item.label ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible translate-y-3'
                      }`}
                      onMouseEnter={() => showMega(item.label)}
                      onMouseLeave={hideMega}
                    >
                      <div className="bg-white rounded-xl shadow-xl ring-1 ring-spe-divider/40 p-2 min-w-[280px]">
                        {megaMenus[item.label].map(sub => (
                          <Link
                            key={sub.to}
                            to={sub.to}
                            className="flex flex-col px-3.5 py-2.5 rounded-lg hover:bg-spe-cream transition-colors duration-200 group"
                          >
                            <span className="text-sm font-medium text-spe-ink group-hover:text-spe-blue transition-colors duration-200">
                              {sub.label}
                            </span>
                            {sub.desc && (
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg transition-colors duration-200 focus-ring ${textMuted} ${hoverBg}`}
                aria-label="Search"
              >
              ...
              <Link
                to="/membership"
                className={`hidden sm:inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 btn-glow ${
                  isTransparent
                    ? 'bg-white text-spe-ink hover:bg-spe-cream shadow-lg shadow-black/15'
              ...
              <button
                ...
                className={`lg:hidden min-w-[44px] min-h-[44px] flex items-center justify-center rounded-lg transition-colors duration-200 ${textMuted} ${hoverBg}`}
        ...
        >
          <div className="bg-white border-t border-spe-divider/50 shadow-xl shadow-black/5">
            <div className="max-w-7xl mx-auto px-4 py-4 space-y-0.5">
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center justify-between px-4 py-3 rounded-lg text-sm font-medium transition-colors duration-200 ${
                        isActive ? 'bg-spe-blue/8 text-spe-blue' : 'text-spe-ink hover:bg-spe-cream'
                      }`
                    }
                  >
                    {item.label}
                    ...
                    <div className="pl-4 space-y-0.5 mb-1">
                      {megaMenus[item.label].map(sub => (
                        <Link
                          ...
                          className="block px-4 py-2.5 text-sm text-spe-muted hover:text-spe-blue hover:bg-spe-cream rounded-lg transition-colors duration-200 min-h-[44px] flex items-center"
                        >
                          {sub.label}
                        </Link>
                      ))}
              ...
              <div className="pt-3 mt-2 border-t border-spe-divider/50 space-y-0.5">
                {[
                  ...
                  <Link
                    ...
                    to={link.to}
                    onClick={() => setMobileOpen(false)}
                    className="block px-4 py-3 text-sm text-spe-muted hover:text-spe-blue hover:bg-spe-cream rounded-lg transition-colors duration-200 min-h-[44px] flex items-center"
                  >
                    {link.label}
                  </Link>
                ))}
              ...
              {/* Mobile CTA */}
              <div className="pt-3 pb-2 border-t border-spe-divider/50 mt-2">
                <Link
                  to="/membership"
                  onClick={() => setMobileOpen(false)}
                  className="block w-full text-center px-4 py-3.5 bg-spe-blue text-white rounded-xl text-sm font-semibold hover:bg-spe-deep transition-colors duration-300 shadow-md shadow-spe-blue/20"
                >
                  Join SPE
                </Link>
              </div>
