...
export default function Footer() {
  ...
  return (
    <footer className="bg-spe-deep2 text-spe-nav-text">
      {/* Newsletter CTA */}
      <div className="bg-gradient-to-r from-spe-deep via-spe-blue to-spe-deep">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
            <div className="text-center lg:text-left">
              <h3 className="text-xl font-serif font-bold text-white">Stay in the loop</h3>
              <p className="text-white/70 text-sm mt-1">Get the latest events, podcasts, and economic insights delivered to your inbox.</p>
            </div>
            {subscribed ? (
              <div className="flex items-center gap-2 text-white">
                <svg className="w-5 h-5 text-spe-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <form onSubmit={handleSubscribe} className="flex w-full max-w-md">
                ...
                <button
                  type="submit"
                  className="px-6 py-3 bg-white text-spe-deep2 text-sm font-semibold rounded-r-lg hover:bg-white/90 transition-colors flex-shrink-0"
                >
                  Subscribe
                </button>
              </form>
      ...
      {/* Sponsors marquee */}
      <div className="border-t border-white/5 bg-spe-deep2/80 overflow-hidden">
        <div className="py-6">
          <p className="editorial-subheading text-center text-white/70 mb-4 text-[10px]">Our Sponsors</p>
          <div className="relative overflow-hidden">
            <div className="marquee-track">
              {[...sponsors, ...sponsors].map((s, i) => (
                <span key={`${s}-${i}`} className="text-sm font-medium text-white/70 hover:text-white transition-colors cursor-default mx-12 whitespace-nowrap flex-shrink-0">
                  {s}
                </span>
              ))}
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        ...
        <div className="mt-14 pt-8 border-t border-white/5 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <p className="text-xs text-white/70">
            Modernised by{' '}
            <a href="https://e17studio.com" target="_blank" rel="noopener noreferrer" className="text-white/70 hover:text-white transition-colors">
              e17studio.com
            </a>
          </p>
        </div>
